# 목표값 근접 상위 N위 경로 분석 (포인트-포인트 / 경유지 포함)
#
# 입력:
#  1) graph_cache.pkl
...
def build_record(route_id, rank, weight_attr, target_value, mode, result, snap_start, snap_end, G):
...
def main():
    try:
        ...
        for i, row in enumerate(routes, start=1):
            ...
            try:
                top_results = choose_target_top_n(G_work, snapped_nodes, target_weight_attr, target_value)
