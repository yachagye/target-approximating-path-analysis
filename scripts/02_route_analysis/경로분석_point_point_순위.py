# 목표 거리/시간 근접 경로 순위 분석 (포인트-포인트 / 경유지 포함)
#
# 입력:
...
#     - 목표값 컬럼은 beg, end 두 개:
#         · beg만 입력(end 빈칸)  → 단일 목표값 모드 (T = beg)
#         · beg, end 모두 입력     → 구간 모드 [beg, end]
#         · beg 빈칸               → 목표값 미입력 → 순위 분석에서 건너뜀(이 스크립트는 route_target만 산출)
#         · beg > end, 또는 end만 입력 → 오류로 해당 행 건너뜀
#  3) 선택 입력: 장애물 GPKG
#     - line 또는 polygon만 지원
#     - 장애물은 완전 차단(교차 간선 제거)
#
# 순위 알고리즘 (목표값 근접 상위 N위 — route_min_* 미산출, route_target 단일 레이어만 출력):
#  - 단일 목표값 모드(choose_target_route_single_top_n):
#      구버전 방출(목표값 근접 abs_diff 순)로 후보를 충분히 수집한 뒤, 방출 결과에
#      다양성 필터(SIM_THRESHOLD)를 적용하여 미세 변형을 제거하고 상위 TOP_N개를 출력.
#      · 1-leg(기점-종점), 2-leg(경유지 1개): abs_diff 순 상위 N이 정확함(브루트포스 검증).
#      · 3-leg 이상(경유지 2개 이상): 구버전 방출(_top_n_three_plus_legs)이 abs_diff 순
#        상위 N을 정확히 보장하지 못함. 출력은 "목표값 근접 영역의 근사치 묶음"으로,
#        대체로 가까운 경로들이나 rank 경계에서 일부 후보가 정확한 순위와 어긋날 수 있음.
#        경유지 2개 이상 경로를 분석할 경우 순위의 엄밀성에 주의.
#      · T < S_min(목표값이 최단 경로보다 작은 경우): 모든 경로가 목표값보다 길어,
#        최소 비용 경로가 곧 최근접 경로이며 소수만 반환됨(정상 동작).
#  - 구간 모드(choose_target_route_interval):
#      [beg, end] 범위 내 후보를 metric 오름차순 수집 → 다양성 필터 → 상위 TOP_N개.
#  ※ 경유지 포함 경로(2-leg 이상)는 leg 병합 시 우회 가능한 구간 왕복(가짜 고리)을
#    가진 후보를 순위 수집에서 제외하여, 상위 N위를 유의미한 경로로만 채운다.
#    진입로가 유일한 막다른 경유지의 불가피한 왕복은 유지한다.
#      범위 내 후보가 없으면 구간 최근접 1개 반환.
#
# 출력:
#  - 경로_point_point_순위.gpkg (EPSG:5179)
#     layer: route_target  (rank 필드 포함, 경로당 최대 TOP_N개 피처.)
...
# 순위 출력 파라미터
TOP_N = 30                 # 다양성 필터 통과 후 최종 출력 수(단일·구간 공통)
...
def choose_target_route_single_top_n(G, snapped_nodes, weight_attr, target_value, top_n=TOP_N):
    """단일 목표값 모드 순위: 목표값 근접 abs_diff 순으로 EMIT_N(500)개를 방출한 뒤,
    그 결과에 다양성 필터(SIM_THRESHOLD)를 적용하여 미세 변형을 제거하고 상위 top_n(30)개를 반환한다.
    '방출 중 필터'가 아니라 '방출 후 필터'. 재방출하지 않는다.

    T < S_min(목표값이 최단 경로보다 작은 경우): 모든 경로가 목표값보다 길어 최소 경로가
    곧 최근접 경로이며, 상위는 미세 변형뿐이므로 최소 경로 1개만 반환한다(운영 지침: 정상 동작).

    Returns: list of dict [{nodes, costs, metric, candidate_n, legs_n}, ...]
    """
    ...
    try:
        for s_, d_ in legs:
            sp = nx.shortest_path(G, source=s_, target=d_, weight=weight_attr)
            s_min += path_metric_from_costs(accumulate_costs(G, sp), weight_attr)
    except (nx.NetworkXNoPath, nx.NodeNotFound):
        return []

    if target_value < s_min:
        emit_n = 1              # T < S_min: 최소 경로 1개만
    else:
        emit_n = EMIT_N         # 방출 500개

    raw = choose_target_top_n(G, snapped_nodes, weight_attr, target_value, top_n=emit_n)
    if not raw:
        return []
...
def choose_target_top_n(G, snapped_nodes, weight_attr, target_value, top_n=TOP_N):
    """타겟 근접 상위 top_n개 경로를 반환한다.

    Returns:
        list of dict: [{nodes, costs, metric, candidate_n, legs_n}, ...]
                      |metric - target| 오름차순 정렬, 최대 top_n개
    """
    legs = list(zip(snapped_nodes[:-1], snapped_nodes[1:]))
    if not legs:
        return []

    print(f"  [target] legs={len(legs)}, weight={weight_attr}, T={target_value:.4f}, top_n={top_n}")

    if len(legs) == 1:
        return _top_n_single_leg(G, legs[0][0], legs[0][1], weight_attr, target_value, top_n)
    else:
        return _top_n_multi_leg(G, legs, weight_attr, target_value, top_n)
...
def _top_n_single_leg(G, src, dst, weight_attr, target_value, top_n):
    ...
    cands.sort(key=lambda x: (abs(x["metric"] - target_value), x["metric"]))
    results = []
...
def _top_n_multi_leg(G, legs, weight_attr, target_value, top_n):
    """경유지 있는 경우: T_j 사전 생성 + bisect 최적화 + lazy iterate.

    2-leg: T_j 작은 leg bisect(사전 생성) + 큰 leg lazy iterate, 양방향 확장으로 top_n 수집.
    3-leg 이상: T_j 사전 생성 + 앞쪽 legs min-heap + 마지막 leg bisect, top_n 수집.
    """
    L = len(legs)
    ...
    for src, dst in legs:
        ...
        min_metrics.append(path_metric_from_costs(leg_min_costs, weight_attr))
    ...
    if L == 2:
        return _top_n_two_legs(G, legs, weight_attr, target_value, top_n, min_metrics, t_j_caps, topo)
    else:
        return _top_n_three_plus_legs(G, legs, weight_attr, target_value, top_n, t_j_caps, topo)


def _top_n_two_legs(G, legs, weight_attr, target_value, top_n, min_metrics, t_j_caps, topo):
    """2-leg top_n: T_j 작은 leg bisect + 큰 leg lazy iterate."""
    ...
    top_heap = []  # max-heap: (-abs_diff, seq, ci, idx_b, path_nodes, costs)
    seq = 0
    iter_n = 0
    t0 = time.time()
    ...
    for path_nodes in gen:
        ...
        for j in range(pos, N_b):
            diff = abs(ci + bm[j] - target_value)
            worst_diff = -top_heap[0][0] if len(top_heap) >= top_n else float("inf")
            if diff > worst_diff:
                break
            chosen_chk = [None, None]
            chosen_chk[li] = {"nodes": path_nodes}
            chosen_chk[bi] = bisect_cands[j]
            if has_artificial_loop(merge_leg_paths(chosen_chk), topo):
...
def _top_n_three_plus_legs(G, legs, weight_attr, target_value, top_n, t_j_caps, topo):
    """3-leg 이상 top_n: T_j 사전 생성 + 앞쪽 legs min-heap + 마지막 leg bisect.

    [정확성 한계] 이 방식은 abs_diff 순 상위 top_n을 정확히 보장하지 못한다.
    앞쪽 legs를 합산 비용 기준 min-heap으로 확장하고 마지막 leg만 bisect하므로,
    abs_diff가 작은 일부 조합(앞쪽 비용이 크지만 마지막 leg에서 보정되는 경우)이
    누락되고 더 먼 조합이 상위에 들 수 있다. 결과 top_n은 목표값 근접 영역의
    '근사치 묶음'이며, rank 경계에서 정확한 순위와 어긋날 수 있다.
    1-leg·2-leg는 정확하나(브루트포스 검증), 3-leg 이상은 근사임에 유의."""
    L = len(legs)
    ...
    while combo_heap:
        ...
        for j in range(pos, N_last):
            diff = abs(c_front + last_metrics[j] - target_value)
            worst_diff = -top_heap[0][0] if len(top_heap) >= top_n else float("inf")
            if diff > worst_diff:
                break
            chosen_chk = [front_cands[k][state[k]] for k in range(F)]
            chosen_chk.append(last_cands[j])
            if has_artificial_loop(merge_leg_paths(chosen_chk), topo):
                continue  # 가짜 고리(non-bridge 왕복) 제외
            if len(top_heap) < top_n:
                heapq.heappush(top_heap, (-diff, seq, c_front + last_metrics[j], state, j))
                seq += 1
            elif diff < -top_heap[0][0]:
                heapq.heapreplace(top_heap, (-diff, seq, c_front + last_metrics[j], state, j))
                seq += 1
...
def collect_interval_two_legs(G, legs, weight_attr, beg, end, top_n, t_j_caps, topo):
...
def collect_interval_three_plus(G, legs, weight_attr, beg, end, top_n, t_j_caps, topo):
...
def choose_target_route_interval(G, snapped_nodes, weight_attr, beg, end, top_n=TOP_N_INTERVAL):
    ...
    if L == 1:
        ...
        if L == 2:
            results, is_fb = collect_interval_two_legs(
                G, legs, weight_attr, beg, end, top_n, t_j_caps, topo,
            )
        else:
...
def build_record(route_id, weight_attr, target_weight_attr, beg, end, mode, result, snap_start, snap_end, G, candidate_n=None, rank=None):
...
def main():
    try:
        ...
        out_gpkg = os.path.join(out_dir, "경로_point_point_순위.gpkg")
        ...
        for i, row in enumerate(routes, start=1):
            ...
            if beg is None:
                if restore_infos:
                    restore_splits(G_work, restore_infos)
                print(f"[{i}/{total}] {route_id} SKIP(목표값 미입력 — 순위 분석은 route_target만 산출)")
                continue
            elif end is None:
                # 단일 목표값 모드 (T = beg): 순위 방출 → 다양성 필터
                try:
                    target_results = choose_target_route_single_top_n(
                        G=G_work,
