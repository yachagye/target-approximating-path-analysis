...
def choose_target_top_n(G, snapped_nodes, dest_nodes, weight_attr, target_value, top_n=TOP_N):
    """타겟 근접 상위 top_n개 경로를 반환한다.

    Returns:
        list of dict: [{nodes, costs, metric, dst_node, candidate_n, legs_n}, ...]
    """
    if len(snapped_nodes) == 1:
        print(f"  [target] direct: weight={weight_attr}, T={target_value:.4f}, top_n={top_n}, dest_nodes={len(dest_nodes)}")
        return _top_n_direct(G, snapped_nodes[0], dest_nodes, weight_attr, target_value, top_n)
    else:
        print(f"  [target] legs={len(snapped_nodes)}(front={len(snapped_nodes)-1}+polygon=1), weight={weight_attr}, T={target_value:.4f}, top_n={top_n}")
        return _top_n_via_waypoints(G, snapped_nodes, dest_nodes, weight_attr, target_value, top_n)
...
def _top_n_direct(G, src_node, dest_nodes, weight_attr, target_value, top_n):
    ...
    all_cands.sort(key=lambda x: x["metric"])

    filtered = []
    for c in all_cands:
        filtered.append(c)
        if c["metric"] > target_value:
            break

    if not filtered:
        filtered = [all_cands[0]]

    filtered.sort(key=lambda x: (abs(x["metric"] - target_value), x["metric"]))
    ...
    for c in filtered[:top_n]:
        results.append({
            ...
            "metric": c["metric"],
            "dst_node": c["dst_node"],
            "candidate_n": len(filtered),
            "legs_n": 1,
        })
    return results


def _top_n_via_waypoints(G, snapped_nodes, dest_nodes, weight_attr, target_value, top_n):
    """유효 좌표 2개 이상: T_j 사전 생성 + bisect 최적화 + lazy iterate, top_n 수집."""
    ...
    if F == 1:
        ...
        for path_nodes in gen:
            ...
            for j in range(pos, N_l):
                diff = abs(cf + lm[j] - target_value)
                wd = -top_heap[0][0] if len(top_heap) >= top_n else float("inf")
                if diff > wd:
                    break
                if has_artificial_loop(merge_leg_paths([{"nodes": path_nodes}, last_candidates[j]]), topo):
        ...
        while combo_heap:
            ...
            for j in range(pos, N_l):
                diff = abs(c_front + lm[j] - target_value)
                wd = -top_heap[0][0] if len(top_heap) >= top_n else float("inf")
                if diff > wd:
                    break
                chosen_chk = [leg_candidates[k][state[k]] for k in range(F)]
                chosen_chk.append(last_candidates[j])
                if has_artificial_loop(merge_leg_paths(chosen_chk), topo):
                    continue  # 가짜 고리(non-bridge 왕복) 제외
                if len(top_heap) < top_n:
                    heapq.heappush(top_heap, (-diff, seq, c_front + lm[j], state, j))
                    seq += 1
                elif diff < -top_heap[0][0]:
                    heapq.heapreplace(top_heap, (-diff, seq, c_front + lm[j], state, j))
                    seq += 1
...
def main():
    try:
        ...
        out_gpkg = os.path.join(out_dir, "경로_point_polygon_순위.gpkg")
