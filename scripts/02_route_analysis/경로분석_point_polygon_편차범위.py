...
def _top_n_direct(G, src_node, dest_nodes, weight_attr, target_value, margin, top_n):
    ...
    # 모든 후보로 '처음 진입'하는 깨끗한 최단경로(core 봉인)를 단일 Dijkstra로 산출 후 margin 범위 필터
    clean = collect_clean_dest_candidates(G, src_node, dest_nodes, dest_nodes_set, weight_attr)
    candidates = [r for r in clean.values() if t_lo <= float(r["metric"]) <= t_hi]
    print(
        f"    [dest_scan] polygon dest={len(dest_nodes)}, clean={len(clean)}, in_range={len(candidates)}, elapsed={time.time() - t0:.1f}s")
